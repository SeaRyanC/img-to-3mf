...
/**
 * Edge vertex indices for a cube
...
/**
 * Marching Cubes algorithm to generate mesh from 3D voxel data
 */
export function marchingCubes(
  ...
  // Iterate through each cube in the volume
  for (let z = 0; z < depth - 1; z++) {
    for (let y = 0; y < height - 1; y++) {
      for (let x = 0; x < width - 1; x++) {
        // Get the 8 corner values of the cube
        const cubeValues = [
          voxelData[x + y * width + z * width * height],
          voxelData[(x + 1) + y * width + z * width * height],
          voxelData[(x + 1) + (y + 1) * width + z * width * height],
          voxelData[x + (y + 1) * width + z * width * height],
          voxelData[x + y * width + (z + 1) * width * height],
          voxelData[(x + 1) + y * width + (z + 1) * width * height],
          voxelData[(x + 1) + (y + 1) * width + (z + 1) * width * height],
          voxelData[x + (y + 1) * width + (z + 1) * width * height],
        ];
        ...
        for (let i = 0; i < 12; i++) {
          if (edges & (1 << i)) {
            const [v1Idx, v2Idx] = edgeConnections[i];
            
            // Get positions of the two vertices
            const v1Pos: [number, number, number] = [
              x + (v1Idx & 1),
              y + ((v1Idx >> 1) & 1),
              z + ((v1Idx >> 2) & 1),
            ];
            const v2Pos: [number, number, number] = [
              x + (v2Idx & 1),
              y + ((v2Idx >> 1) & 1),
              z + ((v2Idx >> 2) & 1),
            ];
            ...
            const vertex = interpolateVertex(
              v1Pos,
              v2Pos,
              cubeValues[v1Idx],
              cubeValues[v2Idx],
        ...
        for (let i = 0; i < tris.length; i += 3) {
          ...
          const normal: [number, number, number] = len > 0.00001
...
export function createColorMask(
  ...
  for (let i = 0; i < width * height; i++) {
    ...
    if (a > 0 && r === targetColor[0] && g === targetColor[1] && b === targetColor[2]) {
      mask[i] = 1;
    }
  }
